...
export function stringsToIntegers(numbers: string[]): number[] {
    const convertedToIntegers = numbers.map((num: string): number =>
        parseInt(num) ? parseInt(num) : 0,
    );
    return convertedToIntegers;
}
...
export const removeDollars = (amounts: string[]): number[] => {
    return amounts.map((amount: string): number => {
        ...
        return parseInt(amount) ? parseInt(amount) : 0;
    });
};
...
export const shoutIfExclaiming = (messages: string[]): string[] => {
    const shouted = messages.filter(
        (message: string): boolean => message[message.length - 1] != "?",
    );
    return shouted.map((message: string): string =>
...
export function injectPositive(values: number[]): number[] {
    ...
    if (findNegative === -1) {
        ...
        const slicedSum = values
            .slice(0, findNegative)
            .reduce((total: number, num: number): number => total + num, 0);
        const slicedArray = [
            ...values.slice(0, findNegative + 1),
            slicedSum,
            ...values.slice(findNegative + 1),
        ];
        return slicedArray;
    }
}
