...
export function stringsToIntegers(numbers: string[]): number[] {
    return numbers.map((num: string) => (+num ? +num : 0));
}
...
export const removeDollars = (amounts: string[]): number[] => {
    const newArr = amounts.map((amount: string) =>
...
export function countShortWords(words: string[]): number {
    const newArr = words.filter((word: string) => word.length < 4);
...
export function makeMath(addends: number[]): string {
    ...
    return addends.length === 0 ? "0=0" : `${total}=${addends.join("+")}`;
}
...
export function injectPositive(values: number[]): number[] {
    ...
    const newSum =
        negIndex === -1 ?
            negArr.reduce((total: number, num: number) => total + num, 0)
        :   negArr
                .slice(0, negIndex)
    ...
    negIndex === -1 ?
        negArr.push(newSum)
    :   negArr.splice(negIndex + 1, 0, newSum);
